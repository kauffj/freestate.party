...
def build():
    ...
    <section class="min-h-screen flex flex-col justify-center px-6 pt-20 pb-12 md:py-0">
        <div class="max-w-6xl mx-auto w-full lg:grid lg:grid-cols-2 lg:gap-12 lg:items-center">
            <div>
                ...
                <a href="{{{{base}}}}/events/" class="inline-block bg-gold-500 hover:bg-gold-400 text-dark-900 font-bold text-lg px-10 py-4 rounded-lg transition-colors min-h-[48px]">
                    Meet Us
                </a>
            </div>
    ...
    # --- Copy slide decks (verbatim — they're prebuilt static HTML, not templated) ---
    slide_routes = []
    if os.path.isdir(SLIDES_DIR):
        for entry in sorted(os.listdir(SLIDES_DIR)):
            src = os.path.join(SLIDES_DIR, entry)
            dst = os.path.join(SITE_DIR, 'slides', entry)
            if os.path.isdir(src):
                if os.path.exists(dst):
                    shutil.rmtree(dst)
                shutil.copytree(src, dst)
                if os.path.exists(os.path.join(dst, 'index.html')):
                    slide_routes.append(f'slides/{entry}')
                print(f"  Copied: slides/{entry}/")
            elif src.endswith(('.html', '.css', '.js', '.svg', '.png', '.jpg', '.webp')):
                os.makedirs(os.path.dirname(dst), exist_ok=True)
                shutil.copy2(src, dst)
                print(f"  Copied: slides/{entry}")

    # --- Generate nginx known-routes.conf ---
    routes_lines = ['# Auto-generated by build.py — do not edit']
    for filepath in pages:
        if filepath == 'index.html':
            routes_lines.append('location = / { try_files /index.html =404; }')
        elif filepath == '404.html':
            continue  # handled by error_page directive
        elif filepath.endswith('/index.html'):
            route = filepath.replace('/index.html', '')
            routes_lines.append(f'location = /{route} {{ try_files /{filepath} =404; }}')
            routes_lines.append(f'location = /{route}/ {{ try_files /{filepath} =404; }}')
    for route in slide_routes:
        routes_lines.append(f'location = /{route} {{ try_files /{route}/index.html =404; }}')
        routes_lines.append(f'location = /{route}/ {{ try_files /{route}/index.html =404; }}')
    routes_conf = '\n'.join(routes_lines) + '\n'
    routes_path = os.path.join(SITE_DIR, 'known-routes.conf')
    with open(routes_path, 'w') as f:
        f.write(routes_conf)
    print(f"  Wrote: site/known-routes.conf ({len(routes_lines) - 1} routes)")
